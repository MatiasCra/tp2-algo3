...
for i in range(test_runtimes.shape[1]):
    plt.plot(test_sizes, test_runtimes[:, i], label=labels[i], linestyle=line_styles[i % len(line_styles)])

# Customize the plot
plt.xlabel("Cantidad de oficinas")
plt.ylabel("Tiempo de ejecución")
plt.title("Tiempo de ejecución con distintas implementaciones de Kruskal")
plt.legend()
