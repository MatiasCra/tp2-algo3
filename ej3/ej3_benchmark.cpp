...
enum Metodo {
    M_DSU_UBR = 0, M_DSU_PC = 1, M_DSU = 2, M_DSU_UBR_PC = 3, M_DENSO = 4
};
...
int main(int argc, char *argv[]) {
    ...
    for (int i = 1; i <= casos_de_test; ++i) {
        ...
        if (metodo == M_DSU_UBR) {
            ...
            aristas.clear();
            obtener_aristas_ralo();
            sort(aristas.begin(), aristas.end());
            double costo_UTP = 0;
            double costo_FO = 0;

            DSU_UBR dsu(cant_oficinas);
            ...
            cout << fixed << setprecision(10) << ms_double.count() / 1000 << endl;
            cout << setprecision(3) << "Caso #" << i << ": " << costo_UTP << " " << costo_FO << endl;
        } else if (metodo == M_DENSO) {

            auto t1 = high_resolution_clock::now();

            obtener_aristas_denso();
            double costo_UTP = 0;
            double costo_FO = 0;
